...
namespace Entropy
{
    public static class LampHooks
    {
        ...
        public static void Update(On.Player.orig_Update orig, Player self, bool e)
        {
            ...
            if (self.objectInStomach != null && self.objectInStomach.type == AbstractPhysicalObject.AbstractObjectType.Lantern && self.room.game.LampWorldState())
            {
                self.Hypothermia -= Mathf.Lerp(-0.0004f, 0f, self.HypothermiaExposure);
            }
        ...
        public static void TerrainImpact(On.Player.orig_TerrainImpact orig, Player self, int chunk, IntVector2 direction, float speed, bool firstContact)
        {
            if (self.IsLampScug())
            {
                ...
                else if (speed > 25f)
                {
                    ...
                    self.Stun(Mathf.Max(50, Mathf.RoundToInt(speed * 2f)));
                }
            }
